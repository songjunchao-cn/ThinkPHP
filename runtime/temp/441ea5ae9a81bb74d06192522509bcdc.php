<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:75:"C:\wamp64\www\tp5\public/../application/admin\view\article\articlelist.html";i:1526516041;s:57:"C:\wamp64\www\tp5\application\admin\view\common\head.html";i:1525311348;}*/ ?>
  <!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>文章管理-VR-FAMILY</title>
...
<body>
        ...
        <div class="page-content">
          <div class="content">
            ...
            <xblock>
              ...
              <button class="layui-btn" onclick="article_add('添加文章','articleadd.html','600','500')"><i class="layui-icon">&#xe608;</i>添加</button>
            </xblock>
            <table class="layui-table">
                ...
                <tbody>
                  <?php if(is_array($articlelist) || $articlelist instanceof \think\Collection || $articlelist instanceof \think\Paginator): $i = 0; $__LIST__ = $articlelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr>
                        <td>
                            <input type="checkbox" id="<?php echo $vo['article_id']; ?>">
                        </td>
                        <td>
                        ...
                        <td >
                            <?php echo $vo['article_content']; ?>
                        </td>
                        <td>
                            <?php echo $vo['article_author']; ?>
                        </td>
                        <td>
                          <?php if($vo['article_pic'] != ''): ?>
                          <img src="http://www.sjc.com/<?php echo $vo['article_pic']; ?>" alt="" width="90">
                          <?php else: ?>
                          无缩略图
                          <?php endif; ?>
                        ...
                        <td class="td-manage">
                            <!-- <a style="text-decoration:none" onclick="article_stop(this,'10001')" href="javascript:;" title="停用">
                                <i class="layui-icon">&#xe601;</i>
                            </a> -->
                            <a title="编辑" href="javascript:;" onclick="article_edit('编辑','articleedit?article_id=<?php echo $vo['article_id']; ?>','500','800')"
                            class="ml-5" style="text-decoration:none">
                                <i class="layui-icon">&#xe642;</i>
                            </a>
                            <!-- <a style="text-decoration:none"  onclick="article_password('修改密码','articlepassword','10001','600','400')"
                            href="javascript:;" title="修改密码">
                                <i class="layui-icon">&#xe631;</i>
                            </a> -->
                            <a title="删除" href="javascript:;" onclick="article_del(this,'<?php echo $vo['article_id']; ?>')"
    ...
    <!-- 底部开始 -->
    <div class="footer">
        <div class="copyright">Copyright ©2017 VR-FAMLIY v2.3 All Rights Reserved.</div>
    </div>
    <!-- 底部结束 -->
    <!-- 背景切换开始 -->
    ...
    laypage.render({
      ...
      jump:function(obj,first){
        console.log(obj);
        if(!first){
          window.location.href="<?php echo url('admin/article/articlelist'); ?>?page=" + obj.curr+'&limit='+obj.limit;
        }
      }
     });
    });
        ...
        function delAll (obj) {
           layer.confirm('确认要删除吗？',function(index){
               ...
               $.ajax({
                   type: 'POST',
                   url:"<?php echo url('Article/articlesdel'); ?>",
                   dataType :'json',
        ...
        function article_stop(obj,id){
            layer.confirm('确认要停用吗？',function(index){
                //发异步把用户状态进行更改
                $(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" onClick="article_start(this,id)" href="javascript:;" title="启用"><i class="layui-icon">&#xe62f;</i></a>');
        ...
        function article_start(obj,id){
            layer.confirm('确认要启用吗？',function(index){
                //发异步把用户状态进行更改
                $(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" onClick="article_stop(this,id)" href="javascript:;" title="停用"><i class="layui-icon">&#xe601;</i></a>');
        ...
        /*密码-修改*/
        function article_password(title,url,id,w,h){
            x_admin_show(title,url,w,h);
        }
        /*用户-删除*/
        function article_del(obj,id){
            ...
            ,function(index){
              $.ajax({
                  type: 'POST',
                  url:"<?php echo url('article/articledel'); ?>",
                  dataType :'json',
                  data:{article_id:id},
                  success:function(data){
                    $(obj).parents("tr").remove();
       ...
       //      btn: ['确定', '取消'],
       //      }, function() {                        // 当确定时执行
       //     $.post("<?php echo url('article/articledel'); ?>", {    // 网址、数据、成功后操作
       //     "_token": "{{ csrf_token() }}",
       //    "_method": "delete"
       //     }, function(data) {
       //     if (data.status == 0) {
